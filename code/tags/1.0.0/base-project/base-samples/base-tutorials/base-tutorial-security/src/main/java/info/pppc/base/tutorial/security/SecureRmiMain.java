...
import info.pppc.base.system.security.ResourceCertificateProvider;
import info.pppc.base.system.util.Logging;
import info.pppc.basex.plugin.discovery.ProactiveDiscovery;
import info.pppc.basex.plugin.modifier.SecureModifier;
...
public class SecureRmiMain {
	...
	public static void main(String[] args) {
		if (args.length != 1) {
			System.out.println("Usage: SecureRmiMain <device1|device2|device3|device4|device5|device6>");
			System.exit(1);
		}	
		// set logging verbosity
		Logging.setVerbosity(Logging.MAXIMUM_VERBOSITY);
		// load the keystore
		KeyStore.initialize(new ResourceCertificateProvider("/certificates/", args[0] + ".der", args[0] + ".key", new String[] { "full/ca.der"} ));
		// add plugins
		...
		m.addPlugin(new SecureModifier());
		m.addPlugin(new ExchangeSemantic());
		...
		try { 
			...
			Logging.error(SecureRmiMain.class, "Could not register service.", e);
			System.exit(1);
		}
			...
			public void perform(IMonitor monitor) throws Exception {
				...
				synchronized (monitor) {
					...
					while (! monitor.isCanceled()) {
						...
						RmiSecureProxy p = new RmiSecureProxy();
		...
		try {
			...
			Logging.error(SecureRmiMain.class, "Thread got interrupted.", e);
		}
		System.exit(0);
	}
