...
public class MxIPBroadcastTransceiver extends IPBroadcastTransceiver implements IMultiplexPlugin {
	...
	private class TransceiverManager implements ITransceiverManager {
		 ...
		 * 
		 * @param connector The connector that has been accepted.
		 */
		public void acceptSession(IStreamConnector connector) {
			synchronized (MxIPBroadcastTransceiver.this) {
				try {
					DataInputStream dis = new DataInputStream(connector.getInputStream());	
					Long key = new Long(dis.readLong());
					MultiplexFactory mux = new MultiplexFactory(MxIPBroadcastTransceiver.this, 
	...
	/**
	 * The ability of the plug-in [1][5].
	 */
	private static final short PLUGIN_ABILITY = 0x0105;
	 ...
	 */
	public MxIPBroadcastTransceiver() {
		super(true);
	}
